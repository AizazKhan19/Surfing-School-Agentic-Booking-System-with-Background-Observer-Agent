...
class ObserverAgent:
    ...
    def _setup_listeners(self):
        ...
        def conversation_item_added(event: ConversationItemAddedEvent):
            ...
            if new_segments >= self.eval_threshold:
                ...
                asyncio.create_task(self._evaluate_with_llm())  # runt he _evaluate_with_llm in the background
    ...
    async def _evaluate_with_llm(self):
        ...
        try:
            ...
            eval_result = self._parse_eval_response(response_text)  # converting the LLM JSON response to structured python dict
    ...
    def _parse_eval_response(self, response_text: str) -> dict | None:
        ...
        #BACKUP FOR THE CASE WHERE LLM RETURNS EXTRA TEXT AROUND JSON
        start = response_text.find('{')
        end = response_text.rfind('}') + 1
        if start >= 0 and end > start:
            try:
                json_str = response_text[start:end]  # removes extra text and extracts JSON portion 
